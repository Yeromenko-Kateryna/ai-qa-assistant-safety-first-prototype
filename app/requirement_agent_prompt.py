...
def build_requirement_agent_prompt(sanitized_input: SanitizedInputEnvelope) -> str:
    ...
    sections = [
        ...
        "- acceptance_criteria: Must be empty array []",
        ...
        "",
        "### FALLBACK VALIDATION RULES FOR OPTIONAL ENTITY ITEMS:",
        "",
        "For this temporary output scope, do not generate optional entity items.",
        "Use this table only if an optional item is emitted despite the scope rule.",
        ...
        "",
        "For this temporary output scope, do not generate items in acceptance_criteria, business_rules, ambiguities, missing_information, or assumptions. Return these lists as [].",
        "If an optional acceptance criterion or business rule is nevertheless emitted as fallback, its provenance must use origin PROPOSED or INFERRED and transformation NONE.",
        "If an optional ambiguity item is emitted despite this scope rule, its description must be non-empty and useful.",
        "TEMPORARY OUTPUT SCOPE RULE: Generate empty arrays [] for: acceptance_criteria, business_rules, ambiguities, missing_information, assumptions. Do not generate any items in these lists for this analysis task. If any optional item is nevertheless emitted, it must be fully valid and will be validated strictly.",
        "",
        "### FORBIDDEN RULES:",
